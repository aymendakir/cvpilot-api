...
class CvController{
 function index(Request $r){return CvDocument::where('user_id',$r->user()->id)->where('expires_at','>',now())->latest()->get();}
 function store(Request $r,DocumentExtractor $extractor){$r->validate(['file'=>'required|file|mimes:pdf,docx,txt|max:10240','is_primary'=>'nullable|boolean']);$file=$r->file('file');$text=$extractor->extract($file);$path=$file->store('cv/'.$r->user()->id,'local');return DB::transaction(function()use($r,$file,$text,$path){if($r->boolean('is_primary'))CvDocument::where('user_id',$r->user()->id)->update(['is_primary'=>false]);$doc=CvDocument::create(['user_id'=>$r->user()->id,'name'=>$file->getClientOriginalName(),'disk_path'=>$path,'mime'=>$file->getMimeType(),'size'=>$file->getSize(),'extracted_text'=>$text,'is_primary'=>$r->boolean('is_primary'),'expires_at'=>now()->addHours(48)]);\App\Services\AdminReview::record('upload',$doc);AuthController::audit($r,'cv_uploaded',$r->user()->id);return response()->json($doc,201);});}
 function analyze(Request $r,CvDocument $cv,AtsScorer $scorer){abort_unless($cv->user_id===$r->user()->id&&$cv->expires_at?->isFuture(),404);$d=$r->validate(['job_description'=>'required|string|min:60|max:30000']);$report=$scorer->score($cv->extracted_text,$d['job_description']);$id=DB::table('ats_reports')->insertGetId(['user_id'=>$r->user()->id,'cv_document_id'=>$cv->id,'score'=>$report['score'],'breakdown'=>json_encode($report['breakdown']),'matched_keywords'=>json_encode($report['matched_keywords']),'missing_keywords'=>json_encode($report['missing_keywords']),'suggestions'=>json_encode($report['suggestions']),'provider'=>'local','created_at'=>now(),'updated_at'=>now()]);AuthController::audit($r,'ats_analyzed',$r->user()->id);return ['id'=>$id]+$report;}
 function download(Request $r,CvDocument $cv){abort_unless($cv->user_id===$r->user()->id&&$cv->expires_at?->isFuture(),404);return Storage::disk('local')->download($cv->disk_path,$cv->name,['Cache-Control'=>'private, no-store']);}
 function destroy(Request $r,CvDocument $cv){abort_unless($cv->user_id===$r->user()->id,404);Storage::disk('local')->delete($cv->disk_path);\App\Services\AdminReview::forget('upload',$cv->id);$cv->delete();return response()->noContent();}
}
